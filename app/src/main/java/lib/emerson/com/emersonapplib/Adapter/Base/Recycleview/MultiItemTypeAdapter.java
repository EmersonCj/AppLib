...
import lib.emerson.com.emersonapplib.Adapter.Base.Recycleview.Item.ItemViewDelegateManager;
...
public class MultiItemTypeAdapter<T> extends RecyclerView.Adapter<RvViewHolder> {
    ...
    protected void setListener(final ViewGroup parent, final RvViewHolder rvViewHolder, int viewType) {
        if (!isEnabled(viewType)) return;
        rvViewHolder.getConvertView().setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View v) {
                if (mOnItemClickListener != null) {
                    int position = rvViewHolder.getAdapterPosition();
                    mOnItemClickListener.onItemClick(v, rvViewHolder, position);
                }
            }
        });
        ...
        rvViewHolder.getConvertView().setOnLongClickListener(new View.OnLongClickListener() {
            ...
            public boolean onLongClick(View v) {
                if (mOnItemClickListener != null) {
                    int position = rvViewHolder.getAdapterPosition();
                    return mOnItemClickListener.onItemLongClick(v, rvViewHolder, position);
                }
                return false;
            }
        });
