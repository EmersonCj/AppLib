...
public class PinYin {
	...
	public static String getPinYin(String src) {
		...
		hpOpf.setCaseType(HanyuPinyinCaseType.LOWERCASE);		//定义汉语拼音大小写类型
		hpOpf.setToneType(HanyuPinyinToneType.WITHOUT_TONE);	//定义汉语拼音声调类型
		hpOpf.setVCharType(HanyuPinyinVCharType.WITH_V);		//定义汉语拼音字符u的类型
		String str = "";
		...
		try {
			for (int i = 0; i < t0; i++) {
				...
				if (Character.toString(charArr[i])
						.matches("[\\u4E00-\\u9FA5]+")) {
					strArr = PinyinHelper.toHanyuPinyinStringArray(charArr[i],hpOpf);// 将汉字的几种全拼都存到t2数组中
