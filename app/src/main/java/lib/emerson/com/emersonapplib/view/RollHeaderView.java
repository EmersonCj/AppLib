...
import lib.emerson.com.emersonapplib.utils.common.DisplayUtil;
...
public class RollHeaderView extends FrameLayout implements OnPageChangeListener {
    ...
    public void setImgUrlData(List<String> urlList) {
        ...
        if (mUrlList != null && !mUrlList.isEmpty()) {
            ...
            for (int i = 0; i < mUrlList.size(); i++) {
                ...
                if (i == 0) {
                    dotIv.setBackgroundResource(R.drawable.indicator_current);
                } else {
                    dotIv.setBackgroundResource(R.drawable.indicator_others);
                }
                //设置点的间距
    ...
    private class MyAdapter extends PagerAdapter {
        ...
        public Object instantiateItem(ViewGroup container, final int position) {
            ...
            Glide.with(mContext).load(mUrlList.get(position % mUrlList.size()))
                    .error(R.mipmap.my_icon).into(iv);
    ...
    public void onPageSelected(int position) {
        dotList.get(prePosition).setBackgroundResource(R.drawable.indicator_current);
        dotList.get(position % dotList.size()).setBackgroundResource(R.drawable.indicator_others);
